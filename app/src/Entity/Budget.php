...
class Budget
{
    ...
    private function calculateTotalIncomesAmount(): float
    {
        return array_reduce(
            $this->incomes->toArray(),
            static fn (float $total, Income $income): float => $total + $income->getAmount(),
            0.0
        );
    }
    ...
    private function calculateTotalExpensesAmount(): float
    {
        return array_reduce(
            $this->expenses->toArray(),
            static fn (float $total, Expense $expense): float => $total + $expense->getAmount(),
            0.0
        );
    }
